...
class MACDBBV2ControllerConfig(DirectionalTradingControllerConfigBase):
    controller_name: str = "macd_bb_v2"
    ...
    position_sizing_method: str = Field(
        default="fix",
        json_schema_extra={
            "prompt": "Enter position sizing method fix or volatility",
            "prompt_on_new": True,
        },
    )
    distribution_method: str = Field(
        default="fibonacci",
        json_schema_extra={
            "prompt": "Enter distribution method: equal, fibonacci, inverted_triangle",
            "prompt_on_new": True,
        },
    )
...
class MACDBBV2Controller(DirectionalTradingControllerBase):
    def __init__(self, config: MACDBBV2ControllerConfig, *args, **kwargs):
        ...
        self.max_records = max(
            config.macd_slow, config.macd_fast, config.macd_signal, config.bb_length
        ) + 20
    ...
    async def update_processed_data(self):
        ...
        # Get volatility factor for dynamic tp or sl
        df["volatility_factor"] = ta.natr(
            high=df["high"],
            low=df["low"],
            close=df["close"],
    ...
    def get_executor_config(self, trade_type: TradeType, price: Decimal, amount: Decimal):
        # Return triple barrier config with adjusted stop loss and take profit based on NATR
        position = PositionExecutorConfig(
            ...
            triple_barrier_config=self.config.triple_barrier_config.new_instance_with_adjusted_volatility(
                self.processed_data["volatility_factor"]
            ),
    ...
    def create_actions_proposal(self) -> List[ExecutorAction]:
        ...
        if signal != 0 and self.can_create_executor(signal):
            ...
            if len(self.get_active_executors()) == 0 and len(self.positions_held) == 0:
                self.logger().info("Open order without position held.")
                ...
                for i in range(1, len(order_sizes) + 1):
                    price_order = (
                        price * (1 + self.config.step_percentage * i)
                        if trade_type == TradeType.SELL
                        else price * (1 - self.config.step_percentage * i)
                    )
                    create_actions.append(
                        CreateExecutorAction(
                            controller_id=self.config.id,
                            executor_config=self.get_executor_config(
                                trade_type, price=price_order, amount=order_sizes[i - 1]
                            ),
                        )
                    )
            else:
                self.logger().info("Open order with position held. Focusing on closing the orders.")
                trade_type = TradeType.BUY if signal > 0 else TradeType.SELL
                create_actions.append(
                    CreateExecutorAction(
                        controller_id=self.config.id,
    ...
    def get_active_executors(self) -> List[ExecutorAction]:
        """
        Retrieves a list of active executors.

        This method filters the executors based on their active status using the provided
        filter function. Only executors marked as active are included in the returned list.

        Returns:
            List[ExecutorAction]: A list of active executors.
        """
    ...
    def stop_action_proposal(self):
        ...
        signal = self.processed_data["signal"]
        if signal == 1:
            self.logger().info(f"Stopping all executors with signal {signal}")
            ...
            self.logger().info("Got short signal, stopping long positions")
            stop_actions.extend(
                self.filter_executors(
                    executors=self.executors_info,
